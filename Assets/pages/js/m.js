...
async function search() {
    ...
    try {
        ...
        const videos = Array.isArray(data) ? data.filter(v => v && v.videoId) : [];
        renderResults(videos);
    } catch (error) {
        console.error("Search failed:", error);
        resultsList.innerHTML = `<li style="grid-column:1/-1;text-align:center;">${SVG_ICONS.warning} Search error: Unable to connect to streaming network.</li>`;
    }
}
...
async function playAudio(videoId, title) {
    ...
    try {
        ...
        console.error("Could not get audio formats:", e);
    }
    if (myToken !== playRequestToken) return;
